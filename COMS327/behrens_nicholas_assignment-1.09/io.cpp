...
void io_object_to_string(object *o,char *s,uint32_t size){
  if(o){
    snprintf(s, size, "%s (speed: %d, dmg: %d+%dd%d)",
...
void io_display_carry(dungeon_t *d){
  clear();
  uint32_t i;
  char s[61];

  for(i=0;i<PC_CARRY_SLOTS;i++){
    io_object_to_string(d->PC->carry_slot[i],s,61);
    mvprintw(7 + i, 10, " %i) %-55s",i,s);
  }

  mvprintw(18, 10, "Hit any key to continue...");
  refresh();
...
void io_display_equipment(dungeon_t *d){
  ...
  for(i=0; i<PC_EQUIPMENT_SLOTS;i++){
    ...
    io_object_to_string(d->PC->equipment_slot[i], t, 61);
    mvprintw(i + 7, 10, "%c %s) %s", 'a'+i,s,t);
  }
  mvprintw(20, 10, "Hit any key to continue...");
...
uint32_t io_take_off_item(dungeon_t *d){
  clear();
  uint32_t i,key;
  char s[61],t[61];
  ...
  mvprintw(20, 10, "Take off which item (ESC to exit)...");
  ...
  while(1){
    ...
    if(key < 'a' || key > 'l'){
      mvprintw(20, 10, "Invalid input. Enter a-l or ESC to cancel.");
      refresh();
      continue;
    }
   if(!d->PC->equipment_slot[key - 'a']){
     mvprintw(20, 10,"Empty equipment slot. Try another.");
     continue;
   }
   if(!d->PC->remove_equipment(key - 'a')){
     return 0;
   }
   snprintf(s, 61, "Can't remove %s. Try again.",d->PC->equipment_slot[key - 'a']->get_name());
   mvprintw(20, 10,"%s",s);
   refresh();
  }
...
uint32_t io_wear(dungeon_t *d){
  ...
  mvprintw(18, 10, "Put on which item (ESC to exit)...");
  ...
  while(1){
    ...
    if(key < '0' || key > '9'){
      mvprintw(18, 10, "Invalid input. Enter 0-9 or ESC to cancel.");
      refresh();
      continue;
    }
   if(!d->PC->carry_slot[key - '0']){
     mvprintw(18, 10,"Empty inventory slot. Try another.");
     continue;
   }
   if(!d->PC->wear_carry(key - '0')){
     return 0;
   }
   snprintf(s, 61, "Can't wear %s. Try again.",d->PC->carry_slot[key - '0']->get_name());
   mvprintw(18, 10,"%s",s);
   refresh();
  }
...
uint32_t io_delete_item(dungeon_t *d){
  clear();
  uint32_t i,key;
  char s[61];
  ...
  mvprintw(18, 10, "Delete which item (ESC to exit)...");
  ...
  while(1){
   ...
   if(!d->PC->destroy_carry(key - '0')){
     return 0;
   }
   snprintf(s, 61, "Can't delete %s. Try again.",d->PC->carry_slot[key - '0']->get_name());
   mvprintw(18, 10,"%s",s);
   refresh();
  }
...
uint32_t io_inspect_item(dungeon_t *d){
  clear();
  uint32_t i,key,l,n;
  char s[80];
  ...
  mvprintw(18, 10, "Inspect which item (ESC to exit)...");
  ...
  while(1){
   ...
   if(!d->PC->carry_slot[key - '0']){
     ...
     clear();
     s[79] = '\0';
     l = strlen(d->PC->carry_slot[key - '0']->get_description());
     for(i = n = 0; i<l;i++){
       if(d->PC->carry_slot[key - '0']->get_description()[i] == '\n'){
         n++;
       }
     }
     ...
     io_object_to_string(d->PC->carry_slot[key - '0'], s, 80);
     mvprintw(1, 0, s);
     mvprintw(3, 0, d->PC->carry_slot[key - '0']->get_description());
     mvprintw(20, 10, "Hit any key to continue...");
     refresh();
     getch();
     return 0;
   }
   snprintf(s, 61, "Can't inspect %s. Try again.",d->PC->carry_slot[key - '0']->get_name());
   mvprintw(18, 10,"%s",s);
   refresh();
  }
...
uint32_t io_drop_carry(dungeon_t *d){
  clear();
  uint32_t i,key;
  char s[61];
  ...
  mvprintw(18, 10, "Drop which item (ESC to exit)...");
  ...
  while(1){
   ...
   if(!d->PC->drop_carry(d, key - '0')){
     return 0;
   }
   snprintf(s, 61, "Can't drop %s. Try again.",d->PC->carry_slot[key - '0']->get_name());
   mvprintw(18, 10,"%s",s);
   refresh();
  }
...
void io_handle_input(dungeon_t *d)
{
  ...
  do {
    ...
    case 'g':
      ...
      break;
    case 'd':
      fail_code = io_drop_carry(d);
      break;
    case 'w':
      fail_code = io_wear(d);
      break;
    case 'x':
      fail_code = io_delete_item(d);
      break;
    case 'I':
      fail_code = io_inspect_item(d);
      break;
    case 'i':
      io_display_carry(d);
      fail_code = 1;
