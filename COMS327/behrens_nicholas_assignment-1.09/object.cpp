...
int32_t object::get_slot_index(){
  if(type < objtype_WEAPON || type > objtype_RING){
    return -1;
  }
  return type - 1;
}
void object::to_pile(dungeon_t *d, int16_t *position){
  next = (object *) d->objmap[position[dim_y]][position[dim_x]];
  d->objmap[position[dim_y]][position[dim_x]] = this;
}
