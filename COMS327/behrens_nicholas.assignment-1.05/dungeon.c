...
int place_stairs(dungeon_t *d){
  pair_t pu,pd;
  pu[dim_y] = rand_range(d->rooms->position[dim_y],
                                     (d->rooms->position[dim_y] +
                                      d->rooms->size[dim_y] - 1));
  pu[dim_x] = rand_range(d->rooms->position[dim_x],
                                     (d->rooms->position[dim_x] +
                                      d->rooms->size[dim_x] - 1));
  mappair(pu) = ter_stairs_up;

  pd[dim_y] = d->pc.position[dim_y];
  pd[dim_x] = d->pc.position[dim_x];

  mappair(pd) = ter_stairs_down;
...
void render_dungeon(dungeon_t *d)
{
  ...
  //Placeholder for the debugger line
  printw("Rendering from ( %d , %d ) to ( %d , %d )",d->renderStart[dim_x],d->renderStart[dim_y],
                                                    d->renderEnd[dim_x],d->renderEnd[dim_y]);
  printw("\n");
  startX = d->renderStart[dim_x];
  startY = d->renderStart[dim_y];
  endX = d->renderEnd[dim_x];
  endY = d->renderEnd[dim_y];
  if(startX < 0){
    startX = 0;
  }
  if(startY < 0){
  ...
  for (p[dim_y] = startY; p[dim_y] < endY; p[dim_y]++) {
    for (p[dim_x] = startX; p[dim_x] < endX; p[dim_x]++) {
      if (charpair(p)) {
        ...
        case ter_debug:
          printw("*");
          //printf(stderr, "Debug character at %d, %d\n", p[dim_y], p[dim_x]);
          break;
        }
      }
    }
  ...
  printw("Up: k, UpLeft: y, UpRight: u, Right: l, Left: h, Down: j, DownLeft: b, DownRight: n");
}
