...
static int32_t tunnel_path_cmp(const void *key, const void *with){
  return ((tunnel_path_t *) key)->cost - ((tunnel_path_t *) with)->cost;
}

void dijkstra_map(dungeon_t *d, pair_t playerPosition){
  static tunnel_path_t path[DUNGEON_Y][DUNGEON_X], *p;
  static uint32_t initialized = 0;
  heap_t h;
  uint32_t x, y;
  int heapCount;

  //Initialize our path to have position values in it.
  if (!initialized) {
  ...
  //Set all the costs to inf before calucation
  for (y = 0; y < DUNGEON_Y; y++) {
    for (x = 0; x < DUNGEON_X; x++) {
      path[y][x].cost = INT_MAX;
    }
  }
  //The cost to go to the player position from the player position is zero
  printf("Player is at ( %d , %d )\n", playerPosition[dim_x], playerPosition[dim_y]);

  path[playerPosition[dim_y]][playerPosition[dim_x]].cost = 0;

  heap_init(&h, tunnel_path_cmp, NULL);
  ...
  printf("Inserted %d values into the heap\n",heapCount);
  ...
  for (y = 0; y < DUNGEON_Y; y++) {
    for (x = 0; x < DUNGEON_X; x++) {
      if(path[y][x].cost != INT_MAX && mapxy(x,y)!=ter_player_pos){
        int lastDigit = path[y][x].cost % 10;
        printf("%d", lastDigit);
      }else if(mapxy(x , y)== ter_player_pos){
        putchar('@');
      }
      else{
        putchar(' ');
...
void calculate_distance_non_tunneling(dungeon_t *d){
  pair_t playerPos;
  uint32_t x,y;
  for (y = 0; y < DUNGEON_Y; y++) {
    for (x = 0; x < DUNGEON_X; x++) {
      if (mapxy(x, y) == ter_player_pos) {
        playerPos[dim_y] = y;
        playerPos[dim_x] = x;
      }
    }
  }
  printf("Got Player Position\n");
  dijkstra_map(d, playerPos);
}

uint8_t hardnessWeight(uint8_t hardness){
  if(hardness >=85 && hardness <=170){
    return 2;
  }
  if(hardness >=171 && hardness <=254){
    return 3;
  }
  return 1;
}

void dijkstra_map_tunnels(dungeon_t *d, pair_t playerPosition){
  static tunnel_path_t path[DUNGEON_Y][DUNGEON_X], *p;
  static uint32_t initialized = 0;
  heap_t h;
  uint32_t x, y;
  ...
  while ((p = heap_remove_min(&h))) {
    ...
    if ((path[p->pos[dim_y] - 1][p->pos[dim_x]    ].hn) &&
        (path[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost >
         p->cost + hardnessWeight(hardnesspair(p->pos)))) {
      path[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost =
        p->cost + hardnessWeight(hardnesspair(p->pos));
      path[p->pos[dim_y] - 1][p->pos[dim_x]    ].from[dim_y] = p->pos[dim_y];
      path[p->pos[dim_y] - 1][p->pos[dim_x]    ].from[dim_x] = p->pos[dim_x];
...
void calculate_distance_tunneling(dungeon_t *d){
  pair_t playerPos;
  uint32_t x,y;
  for (y = 0; y < DUNGEON_Y; y++) {
    for (x = 0; x < DUNGEON_X; x++) {
  ...
  printf("Got Player Position\n");
  dijkstra_map_tunnels(d, playerPos);
}
...
void usage(char *name)
{
  fprintf(stderr,
          ...
          "          [-p|--player [<Xcoord>] [<Ycoord>]]\n",
          name);

  exit(-1);
}

void place_PC_from_load(dungeon_t *d, int xCoord,int yCoord){
  pair_t playerPosition;

  playerPosition[dim_x] = yCoord;
  playerPosition[dim_y] = xCoord;

  mappair(playerPosition) = ter_player_pos;
}

void place_PC(dungeon_t *d){
  pair_t playerPosition;
  int randomRoom = rand_range(0,d->num_rooms);

  int randomPositionX = rand_range(d->rooms[randomRoom].position[dim_x],
                    d->rooms[randomRoom].position[dim_x]+d->rooms[randomRoom].size[dim_x]-1);

  int randomPositionY = rand_range(d->rooms[randomRoom].position[dim_y],
                    d->rooms[randomRoom].position[dim_y]+d->rooms[randomRoom].size[dim_y]-1);

  playerPosition[dim_y] = randomPositionY;
  playerPosition[dim_x] = randomPositionX;
...
int main(int argc, char *argv[])
{
  ...
  struct timeval tv;
  uint32_t i;
  uint32_t do_load, do_save, do_seed, do_image,do_player_load;
  uint32_t long_arg;
  char *save_file;
  char *load_file;
  char *pgm_file;
  int load_xcoord,load_ycoord;
 ...
 if (argc > 1) {
      ...
      if (argv[i][0] == '-') { /* All switches start with a dash */
        ...
        case 'p':
          ...
          if ((argc > i + 1) && argv[i + 1][0] != '-') {
            /* There is another argument, and it's not a switch, so *
             * we'll treat it as a save file and try to load it.    */
            load_xcoord = atoi(argv[++i]);
            load_ycoord = atoi(argv[++i]);
          }
          break;
        default:
          usage(argv[0]);
  ...
  if(do_player_load){
    place_PC_from_load(&d,load_xcoord,load_ycoord);
  }else{
    place_PC(&d);
  }
  ...
  calculate_distance_non_tunneling(&d);

  calculate_distance_tunneling(&d);
