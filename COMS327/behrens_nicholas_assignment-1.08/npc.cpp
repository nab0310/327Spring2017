...
npc_characteristics_t npc_get_characteristics(npc *c){
  return c->characteristics;
}
uint32_t npc_get_color(npc *c){
  return c->color;
}
...
void gen_monsters(dungeon_t *d)
{
  ...
  std::vector<npc*> &mon = d->monsters;

  std::vector<npc*>::iterator ni;

  for(i=0;i<d->max_monsters;i++){
    int monsterNum = rand_range(1, d->monsters.size());
    ni = mon.begin();
    while(monsterNum > 1){
      ni++;
      monsterNum--;
    ...
    m->speed = character_get_speed(*ni);
    m->color = npc_get_color(*ni);
    m->alive = 1;
    m->sequence_number = ++d->character_sequence_number;
    m->characteristics = npc_get_characteristics(*ni);
    m->symbol = character_get_symbol(*ni);
    m->have_seen_pc = 0;
