...
void io_display(dungeon_t *d)
{
  ...
  for (y = 0; y < 21; y++) {
    for (x = 0; x < 80; x++) {
      ...
      if (d->object_map[d->io_offset[dim_y] + y]
                          [d->io_offset[dim_x] + x]) {
          if(can_see(d,
                      character_get_pos(d->PC),
                      object_get_postition(d->object_map[d->io_offset[dim_y] + y]
                                          [d->io_offset[dim_x] + x]),1)){
                                            d->object_map[d->io_offset[dim_y] + y]
                                                                [d->io_offset[dim_x] + x]->have_seen_pc = 1;
                                          }
          if(d->object_map[d->io_offset[dim_y] + y]
                              [d->io_offset[dim_x] + x]->have_seen_pc){
      ...
      else if (d->character_map[d->io_offset[dim_y] + y]
                          [d->io_offset[dim_x] + x] &&
          can_see(d,
                  character_get_pos(d->PC),
...
void io_handle_input(dungeon_t *d)
{
  ...
  do {
    ...
    case 'q':
      ...
      io_queue_message("There are %s monsters.", d->monsters.size());
