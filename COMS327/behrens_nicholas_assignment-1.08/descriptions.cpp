                                          ...
                                          dungeon_t *d)
{
  ...
  m.create(name, desc, symb, color, speed, abil, hp, dam, d);
  v->push_back(m);
                                         ...
                                         dungeon_t *d)
{
  ...
  o.create(name, desc, type, color, hit, dam,
        dodge, def, weight, speed, attr, val, d);
  v->push_back(o);
                                           ...
                                           std::vector<monster_description> *v)
{
  ...
  f >> lookahead;
  do {
    parse_monster_description(f, &lookahead, v, d);
  } while (f.peek() != EOF);
                                          ...
                                          std::vector<object_description> *v)
{
  ...
  f >> lookahead;
  do {
    parse_object_description(f, &lookahead, v,d);
  } while (f.peek() != EOF);
...
uint32_t print_descriptions(dungeon_t *d)
{
  ...
  for (mi = m.begin(); mi != m.end(); mi++) {
    npc* c = new npc(mi->get_name(), mi->get_symbol(), mi->get_color(),
                      mi->get_speed().roll(), mi->get_abilities(),
                      mi->get_hitpoints().roll(), mi->get_damage());
    d->monsters.push_back(c);
  }

  for (oi = o.begin(); oi != o.end(); oi++) {

    for (i = 0; types_lookup[i].name; i++) {
      if (oi->get_type() == types_lookup[i].value) {
        objectSymbol = object_symbol[i];
        break;
      }
    }
    dungeon_object* d_o = new dungeon_object(oi->get_name(), oi->get_type(),
                        oi->get_color(), oi->get_hit().roll(), oi->get_damage(),
                        oi->get_dodge().roll(),oi->get_defence().roll(),
                        oi->get_weight().roll(), oi->get_speed().roll(),
                        oi->get_attribute().roll(), oi->get_value().roll(),
                        objectSymbol);
    d->dungeon_objects.push_back(d_o);

  }

  std::cout << "size of object vector after printing is: " << d->dungeon_objects.size() << std::endl;


  return 0;
}

void monster_description::create(const std::string &name,
...
void object_description::create(const std::string &name,
                             const std::string &description,
