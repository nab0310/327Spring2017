...
{
  if (is_character_alive(def)) {
    character_set_alive(def, 0);
    if (def != d->pc) {
      d->num_monsters--;
    }
    character_set_kills(atk, kill_direct, character_get_kills(atk, kill_direct)+1);
    character_set_kills(atk, kill_avenged, character_get_kills(atk, kill_avenged) +
                                          character_get_kills(def, kill_direct) +
                                          character_get_kills(def, kill_avenged));
  }
