...
void load_hardness(unsigned char bufferHardness[16800], dungeon_t *d){
  int i,x,y;
  i=0;
  for (y = 0; y < DUNGEON_Y; y++) {
    for (x = 0; x < DUNGEON_X; x++) {
      d->hardness[y][x] = bufferHardness[i];
      i++;
    }
  }
}

void load_rooms(dungeon_t *d, uint8_t *roomInfo){
  int i =0;
  for(i=0;i<d->num_rooms;i++){
    d->rooms[i].position[dim_x] = roomInfo[4*i];
    d->rooms[i].position[dim_y] = roomInfo[4*i+1];
    d->rooms[i].size[dim_x] = roomInfo[4*i+2];
    d->rooms[i].size[dim_y] = roomInfo[4*i+3];
  }
}

void setRoomNumber(dungeon_t *d , int roomNumber){
  d->num_rooms = roomNumber;
}

int getRoomNumber(dungeon_t *d){
  return d->num_rooms;
}

void getFloorOfRooms(dungeon_t *d){
  pair_t p;
  int i;
  int y=0;
  int x=0;
  ...
  for(y=0;y<DUNGEON_Y;y++){
    for(x =0;x<DUNGEON_X;x++){
      ...
      if(d->hardness[y][x] == 0){
        for ( i = 0; i < d->num_rooms; i++){
          if(p[dim_x] >= d->rooms[i].position[dim_x]+1 &&
            p[dim_x] <= d->rooms[i].position[dim_x] + d->rooms[i].size[dim_x]){
              if(p[dim_y] >= d->rooms[i].position[dim_y] &&
                p[dim_y] <= d->rooms[i].position[dim_y] + d->rooms[i].size[dim_y]-1){
                  mappair(p) = ter_floor_room;
                  break;
          }
        }
      }
      if(mappair(p) != ter_floor_room){
...
void write_hardness_to_buffer(dungeon_t *d, FILE *fp){
  uint8_t hardnessBuffer[16800];
  int i=0;
  int y=0;
  int x=0;
  for(y=0;y<DUNGEON_Y;y++){
    for(x=0;x<DUNGEON_X;x++){
      hardnessBuffer[i] = (d->hardness[y][x]);
      i++;
    }
  }
  fwrite(hardnessBuffer,1,16800,fp);
}

void write_rooms_to_buffer(dungeon_t *d, FILE *fp){
  uint8_t xNum[1];
  uint8_t yNum[1];
  uint8_t width[1];
  uint8_t height[1];
  int i=0;
  for(i=0; i< d->num_rooms;i++){
    xNum[0] = d->rooms[i].position[dim_x];
    yNum[0] = d->rooms[i].position[dim_y];
    width[0] = d->rooms[i].size[dim_x];
    height[0] = d->rooms[i].size[dim_y];
    fwrite(xNum,1,1,fp);
    fwrite(yNum,1,1,fp);
    fwrite(width,1,1,fp);
...
int main(int argc, char *argv[])
{
  ...
  if(loadGame){
    //load_dungeon(&d);
    //printf("This will load a game\n");
    FILE *fp;
    char fileName[40] = "";
    char *inputFilePath = getenv("HOME");
    strcat(fileName, inputFilePath);
    strcat(fileName,"/.rlg327/");
    char inputFilename[20] = "dungeon.rlg327";
    fp = fopen(strcat(fileName,inputFilename), "rb");
    if (fp == NULL) {
      ...
      fseek(fp, 0,SEEK_END);
      long lastByte = ftell(fp);
      fseek(fp,SEEK_SET,0);
      char buffer[20];
      fread(buffer, 12, 1, fp);
      if(strcmp(buffer, "RLG327-S2017")){
        printf("Cannot load file, error in file type marker!\n");
        printf("File maker was %s\n", buffer);
        exit(1);
      }
      unsigned int bufferInt[10];
      fread(bufferInt, 4,1,fp);
      if(bufferInt[0] != 0){
        printf("Cannot load file, error in file version marker!\n");
        exit(1);
      }
      fread(bufferInt, 3,1,fp);
      if(bufferInt[0] == 0){
        printf("Cannot load file, error in file size marker!\n");
        exit(1);
      }
      //Hardness by values
      unsigned char bufferHardness[16800];
      fread(bufferHardness, 1, 16801,fp);
      //Load hardness values into hardness array
      load_hardness(bufferHardness,&d);
      //Rooms
      int sizeOfRooms = (int) lastByte - 16820;
      uint8_t roomsInfo[sizeOfRooms];
      fread(roomsInfo, sizeof(uint8_t), sizeOfRooms,fp);
      int roomIndex =0;
      int i=0;
      for(i=0;i<sizeOfRooms;i+=4){
        //load_room(&d, roomsInfo[i], roomsInfo[i+1],roomsInfo[i+2],roomsInfo[i+3], roomIndex);
        roomIndex++;
      }
      setRoomNumber(&d,roomIndex);
      load_rooms(&d,roomsInfo);
      //Load rooms into room property of the dungeon
      fclose(fp);
      getFloorOfRooms(&d);
    }
  }else{
  ...
  if(saveGame){
    FILE *fp;
    char fileName[40] = "";
    char *inputSaveFilePath = getenv("HOME");
    strcat(fileName, inputSaveFilePath);
    strcat(fileName,"/.rlg327/");
    char inputSaveFilename[] = "dungeon.rlg327";
    fp = fopen(strcat(fileName,inputSaveFilename), "w");
    if (fp == NULL) {
      ...
      //printf("File is open.\n");
      fprintf(fp, "RLG327-S2017");
      uint32_t buffer[] = { htobe32(0) };
      fwrite (buffer , 4, 1, fp);
      //write the size of the file (use the number of rooms to caluclate)
      uint32_t sizeNum = 16820 + 4*(getRoomNumber(&d));
      //printf("Size of the file is: %d\n", sizeNum);
      uint32_t size[] = { htobe32(sizeNum) };
      fwrite(size, 4, 1,fp);
      //fwrite (buffer , sizeof(uint32_t), sizeof(buffer), fp);
      write_hardness_to_buffer(&d, fp);
      write_rooms_to_buffer(&d, fp);
      fclose(fp);
    }
  }
  delete_dungeon(&d);
