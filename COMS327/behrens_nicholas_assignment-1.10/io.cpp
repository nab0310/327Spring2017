    ...
    for (pos[dim_x] = -PC_VISUAL_RANGE;
      ...
      if (d->character_map[d->io_offset[dim_y] +
        ...
        case ter_stairs_down:
          ...
          break;
        case ter_poisoned:
            mvaddch(pos[dim_y] + 1, pos[dim_x], 'p');
            break;
        default:
...
void io_display_attack_square(dungeon_t *d,pair_t attackSquare)
{
  pair_t pos;
  uint32_t illuminated;
  uint32_t color;
      ...
      if (d->character_map[d->io_offset[dim_y] + pos[dim_y]]
          ...
          can_see(d,
                  ...
                  character_get_pos(d->character_map[d->io_offset[dim_y] +
                                                    ...
                                                    [d->io_offset[dim_x] +
                                                     pos[dim_x]]), 1, 0)) {
         if(d->io_offset[dim_y] + pos[dim_y] == d->PC->position[dim_y] + attackSquare[dim_y] &&
             d->io_offset[dim_x] + pos[dim_x] == d->PC->position[dim_x] + attackSquare[dim_x]){
...
int io_ranged_combat(dungeon_t *d){
  mvprintw(10, 18, "Pick which square you want to attack... r to confirm... ESC to quit.");
  refresh();
  pair_t attackSquare;
  int key;
  uint32_t i;
  ...
  attackSquare[dim_x] = 0;
  attackSquare[dim_y] = 0;
    ...
    while(1){
      ...
      switch (key) {
        ...
        case KEY_UP:
          attackSquare[dim_y]--;
          io_display_attack_square(d, attackSquare);
          break;
        case '6':
        case 'l':
        case KEY_RIGHT:
        ...
        case KEY_LEFT:
          ...
          break;
        case 'r':
        if(d->character_map[d->PC->position[dim_y] + attackSquare[dim_y]]
                            [d->PC->position[dim_x] + attackSquare[dim_x]]){
          if(can_see(d,
                  character_get_pos(d->PC),
                  character_get_pos(d->character_map[d->PC->position[dim_y] +
                                                     attackSquare[dim_y]]
                                                    [d->PC->position[dim_x] +
                                                     attackSquare[dim_x]]), 1, 0)){
                       hasRangedWeapon = false;
                       for (i = 0; i < num_eq_slots; i++) {
                         if (d->PC->eq[i]) {
                           if(d->PC->eq[i]->get_type() == objtype_RANGED){
                             hasRangedWeapon = true;
                           }
                         }
                       }
                       if(hasRangedWeapon){
                         do_ranged_combat(d,d->PC,d->character_map[d->PC->position[dim_y] + attackSquare[dim_y]]
                                                           [d->PC->position[dim_x] + attackSquare[dim_x]]);
                         return 0;
                       }else{
                         mvprintw(10, 18, "You have no ranged weapon!");
                         refresh();
                       }
                 }else{
                   mvprintw(10, 18,"You cannot see that square.... Try again!");
                   refresh();
                 }
        }else{
          mvprintw(10, 18,"There is nothing there... Try again!");
          refresh();
        }
          break;
      }
...
void io_display_poison_square(dungeon_t *d,pair_t attackSquare)
{
  pair_t pos;
  uint32_t illuminated;
  uint32_t color;
  int32_t squareSubY, squareSubX;

  clear();
  for (pos[dim_y] = 0; pos[dim_y] < 21; pos[dim_y]++) {
    for (pos[dim_x] = 0; pos[dim_x] < 80; pos[dim_x]++) {
      squareSubY = d->io_offset[dim_y] + pos[dim_y] - d->PC->position[dim_y] + attackSquare[dim_y];
      squareSubX = d->io_offset[dim_x] + pos[dim_x] - d->PC->position[dim_x] + attackSquare[dim_x];
      if ((illuminated = is_illuminated(d->PC,
                                        d->io_offset[dim_y] + pos[dim_y],
                                        d->io_offset[dim_x] + pos[dim_x]))) {
        attron(A_BOLD);
      ...
      if (d->character_map[d->io_offset[dim_y] + pos[dim_y]]
          ...
          can_see(d,
                  ...
                  character_get_pos(d->character_map[d->io_offset[dim_y] +
                                                    ...
                                                    [d->io_offset[dim_x] +
                                                     pos[dim_x]]), 1, 0)) {
         if((squareSubY < POISON_RADIUS && squareSubY > -POISON_RADIUS ) &&
             (squareSubX < POISON_RADIUS && squareSubX > -POISON_RADIUS)){
               attron(COLOR_PAIR(color = COLOR_RED));
         }else{
          attron(COLOR_PAIR((color =
                             d->character_map[d->io_offset[dim_y] + pos[dim_y]]
...
void poison_affected_area(dungeon_t *d, pair_t attackSquare){
  int x = d->PC->position[dim_x] - attackSquare[dim_x];
  int y = d->PC->position[dim_y] - attackSquare[dim_y];
  if(mapxy(x,y) != ter_wall){
      mapxy(x, y) = ter_poisoned;
  }
  if(mapxy(x,y+1) != ter_wall){
      mapxy(x, y+1) = ter_poisoned;
  }
  if(mapxy(x,y-1) != ter_wall){
      mapxy(x, y-1) = ter_poisoned;
  }
  if(mapxy(x+1,y) != ter_wall){
      mapxy(x+1, y) = ter_poisoned;
  }
  if(mapxy(x+1,y+1) != ter_wall){
      mapxy(x+1, y+1) = ter_poisoned;
  }
  if(mapxy(x+1,y-1) != ter_wall){
      mapxy(x+1, y-1) = ter_poisoned;
  }
...
int io_poison_ball(dungeon_t *d){
  mvprintw(10, 18, "Pick which square you want to poison... p to confirm.... ESC to quit.");
  refresh();
  pair_t attackSquare;
  int key;
  ...
  attackSquare[dim_y] = 0;
    ...
    while(1){
      ...
      switch (key) {
        ...
        case KEY_UP:
          attackSquare[dim_y]++;
          io_display_poison_square(d, attackSquare);
          break;
        case '6':
        case 'l':
        case KEY_RIGHT:
        ...
        case KEY_LEFT:
          ...
          break;
        case 'p':
          poison_affected_area(d,attackSquare);
          io_display(d);
          return 0;
        break;
...
void io_handle_input(dungeon_t *d)
{
  ...
  do {
    ...
    case 'I':
      ...
      break;
    case 'r':
      fail_code = io_ranged_combat(d);
      break;
    case 'p':
      fail_code = io_poison_ball(d);
      break;
    case 'q':
